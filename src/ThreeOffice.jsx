...
export default function ThreeOffice({agents, chosen, selected, setSelected, paused, setPaused, speed, setSpeed, progress, officeTime, setReportOpen, reportOpen, reportsCount, reports, filter, setFilter, activeReport, setActiveReport}) {
  const mount=useRef(null), groups=useRef(new Map()), agentsRef=useRef(agents)
  useEffect(()=>{
    ...
    let raf; const animate=()=>{raf=requestAnimationFrame(animate);controls.update();groups.current.forEach(g=>{const a=agentsRef.current.find(x=>x.id===g.userData.agentId);if(!a)return;const tx=(a.x-50)/5.3,tz=(a.y-50)/5.3;g.position.x+=(tx-g.position.x)*.08;g.position.z+=(tz-g.position.z)*.08;g.userData.tag.position.x=g.position.x;g.userData.tag.position.z=g.position.z;g.userData.tag.material.opacity=a.id===selected?1:.82});renderer.render(scene,camera)}; animate()
